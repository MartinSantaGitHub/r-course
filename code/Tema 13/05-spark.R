#library(devtools)
devtools::install_github("apache/spark@v3.1.1", 
                         subdir = "R/pkg")
...
sub.iris.df <- select(iris.df,c("Sepal_Length","Petal_Length"))
...
collect(sql(paste("select Species, avg(Sepal_Length) avg_sl, ", 
                  "avg(Sepal_Width) avg_sw FROM IrisTable ", 
                  "GROUP BY Species order by avg_sl desc")))
